      ...
      <div className='flex justify-between items-center max-w-6xl mx-auto p-3'>
        <Link to='/'>
          <h1 className='font-bold text-sm sm:text-xl flex flex-wrap'>
            <span className='text-slate-300'>Byte</span>
        ...
        >
          <input
            type='text'
            placeholder='Search...'
            className='bg-transparent focus:outline-none w-24 sm:w-64 placeholder-slate-300 text-slate-100'
            value={searchTerm}
        ...
        <ul className='flex gap-4 items-center'>
          ...
          {location.pathname === '/' && currentUser && (
            <Link to='/create-listing'>
              <span className='bg-blue-600 text-white px-3 py-2 rounded-lg text-sm hover:opacity-95'>
                Create Listing
              </span>
            </Link>
          )}
